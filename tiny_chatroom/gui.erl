...
-record(state, {
    ...
    client,
    gui,
    % unique part of name, as integer
    clientid
}).
        ...
        {result, ClientPID, Ref, Response} ->
            case Response of
                ...
                err ->
                    ...
                    write_chatroom(
                        with_label(ClientName, ?SYSTEM), "+ Already in chatroom " ++ ChatName
                    );
                ...
                History ->
                    ...
                    lists:foreach(
                        fun({Nickname, Message}) ->
                            write_chatroom(
                                with_label(ClientName, chatroom_id(ChatName)),
                                Nickname ++ ": " ++ Message
                            )
                        end,
                        History
        ...
        {result, ClientPID, Ref, Response} ->
            case Response of
                ...
                err ->
                    ...
                    write_chatroom(
                        with_label(ClientName, ?SYSTEM),
                        "+ Cannot leave chatroom " ++ ChatName ++ ". not in chatroom!"
                    )
            end
    end.
...
do_send_msg(Ref, ClientName, ClientPID, Message) ->
    ...
    case ChatName of
        ...
        "System" ->
            write_chatroom(
                with_label(ClientName, ?SYSTEM), "+ Cannot send messages to " ++ ChatName
            );
        ...
        _ ->
            ...
            receive
                {result, ClientPID, Ref, {_, Nickname}} ->
                    %% write to chatroom
                    write_chatroom(
                        with_label(ClientName, chatroom_id(ChatName)), Nickname ++ ": " ++ Message
                    )
            end
    end.
...
do_whoami(St, Ref, ClientPID) ->
    ...
    receive
        {result, ClientPID, Ref, Nickname} ->
            ...
            write_chatroom(
                with_label(St#state.client, ?SYSTEM),
        ...
        {result, ClientPID, Ref, Response} ->
            ...
            case Response of
                ...
                err_same ->
                    write_chatroom(
                        with_label(St#state.client, ?SYSTEM),
                        "+ Error: Could not change nickname. Requested nickname same as current nickname!"
                    );
                %% nickname is used by someone else
                err_nick_used ->
                    write_chatroom(
                        with_label(St#state.client, ?SYSTEM),
                        "+ Error: Could not change nickname. Requested nickname already used by another client!"
                    );
                %% we successfully changed nickname
                ok_nick ->
                    write_chatroom(
                        with_label(St#state.client, ?SYSTEM),
                        "+ Nickname updated! New nickname: \"" ++ Nick ++ "\"."
                    )
            end
    end.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
handle_event(
    #wx{event = #wxCommand{type = command_text_enter, cmdString = Item}},
    ...
    case Cmd of
        ...
        {ignore, Line} ->
            write_chatroom(
                with_label(St#state.client, ?SYSTEM), "+ Command not recognized: " ++ Line
            )
    end,
    focus(with_label(ClientName, ?CMDLINE)),
    {noreply, St};
...
handle_event(
    #wx{event = #wxAuiNotebook{type = command_auinotebook_button, selection = _TabPos}},
