...
do_join(State, Ref, ChatName) ->
    % check if the client is already joined the chatroom
        ...
        false ->
            ...
            receive
                {ChatroomPid, Ref, connect, History} ->
                    ...
                    {History, NewState};
                % haven't found chatroom named ChatName
                {server, Ref, {error, not_found}} ->
                    {err, State}
            end
    end.
...
do_new_nick(State, Ref, NewNick) ->
    % check if the new nickname is the same with the currently used
...
do_msg_send(State, Ref, ChatName, Message) ->
    % get the chatroom PID and send it message
